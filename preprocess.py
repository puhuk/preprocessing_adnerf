...
body_imgs_dir = os.path.join('dataset', id, 'body_imgs')
Path(body_imgs_dir).mkdir(parents=True, exist_ok=True)
...
print("valid_img_ids", valid_img_ids[0])
...
# Step 3: face parsing
if running_step == 3:
    # python simple_extractor.py --dataset 'pascal' --model-restore 'checkpoints/exp-schp-201908270938-pascal-person-part.pth' --input-dir '../AD-NeRF/dataset/shelley_1/ori_imgs' --output-dir 'dataset/shelley_1/parsing'
    print('--- Step 3: face parsing ---')
    # os.system('cd ../Self-Correction-Human-Parsing')
    # os.system('cd ../Self-Correction-Human-Parsing')
    # os.system('mkdir outputs/' + id)
    face_parsing_cmd = "python ../Self-Correction-Human-Parsing/simple_extractor.py --dataset 'pascal' --model-restore 'checkpoints/exp-schp-201908270938-pascal-person-part.pth' --input-dir '../AD-NeRF/dataset/" \
        + id + "/ori_imgs' --output-dir 'dataset/" + id + "/parsing'"
    print(face_parsing_cmd)
...
if running_step == 4:
    ...
    distss = []
    for i in sel_ids:
        print(os.path.join(id_dir, 'parsing', str(i) + '.png'))
        parse_img = cv2.imread(os.path.join(id_dir, 'parsing', str(i) + '.png'))
        print(parse_img.shape)
        # bg = (parse_img[..., 0] == 255) & (
        #     parse_img[..., 1] == 255) & (parse_img[..., 2] == 255)
        bg = (parse_img[..., 0] == 0) & (parse_img[..., 1] == 0) & (parse_img[..., 2] == 0)
        # hand = ((parse_img[..., 0] == 128) & (parse_img[..., 1] == 0) & (parse_img[..., 2] == 0)).astype(np.float32)
        # torso = ((parse_img[..., 0] == 0) & (parse_img[..., 1] == 128) & (parse_img[..., 2] == 0)).astype(np.float32)
        # head =  ((parse_img[..., 0] == 0) & (parse_img[..., 1] == 0) & (parse_img[..., 2] == 128)).astype(np.float32)
        # arm =  ((parse_img[..., 0] == 0) & (parse_img[..., 1] == 128) & (parse_img[..., 2] == 128)).astype(np.float32)
        # leg =  ((parse_img[..., 0] == 128) & (parse_img[..., 1] == 0) & (parse_img[..., 2] == 128)).astype(np.float32)
        # foot =  ((parse_img[..., 0] == 128) & (parse_img[..., 1] == 128) & (parse_img[..., 2] == 0)).astype(np.float32)
...
if running_step == 5:
    ...
    for i in valid_img_ids:
        ...
        head_part = (parsing_img[:, :, 0] == 0) & (
            parsing_img[:, :, 1] == 0) & (parsing_img[:, :, 2] == 128)
        bc_part = (parsing_img[:, :, 0] == 0) & (
            parsing_img[:, :, 1] == 0) & (parsing_img[:, :, 2] == 0)
        bd_part = ((parsing_img[..., 0] == 128) & (parsing_img[..., 1] == 0) & (parsing_img[..., 2] == 0)) & ((parsing_img[..., 0] == 0) & (parsing_img[..., 1] == 128) & (parsing_img[..., 2] == 0)) & ((parsing_img[..., 0] == 0) & (parsing_img[..., 1] == 128) & (parsing_img[..., 2] == 128)) & ((parsing_img[..., 0] == 128) & (parsing_img[..., 1] == 0) & (parsing_img[..., 2] == 128)) & ((parsing_img[..., 0] == 128) & (parsing_img[..., 1] == 128) & (parsing_img[..., 2] == 0))
        ...
        img[bc_part] = bc_img[bc_part]
        img[head_part] = bc_img[head_part]
        cv2.imwrite(os.path.join(body_imgs_dir, str(i) + '.jpg'), img)
...
if running_step == 8:
    ...
    for i in range(2):
        ...
        for i in ids:
            ...
            frame_dict['face_rect'] = rect.tolist()
            transform_dict['frames'].append(frame_dict)
        with open(os.path.join(id_dir, 'body_transforms_' + save_id + '.json'), 'w') as fp:
            json.dump(transform_dict, fp, indent=2, separators=(',', ': '))

    dir_path = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    testskip = int(val_ids.shape[0]/7)

    HeadNeRF_config_file = os.path.join(id_dir, 'body_HeadNeRF_config.txt')
    with open(HeadNeRF_config_file, 'w') as file:
        file.write('expname = ' + id + '_head\n')
        file.write('datadir = ' + os.path.join(dir_path, 'dataset', id) + '\n')
        file.write('basedir = ' + os.path.join(dir_path,
    ...
    ComNeRF_config_file = os.path.join(id_dir, 'body_TorsoNeRF_config.txt')
    with open(ComNeRF_config_file, 'w') as file:
        file.write('expname = ' + id + '_com\n')
        file.write('datadir = ' + os.path.join(dir_path, 'dataset', id) + '\n')
        file.write('basedir = ' + os.path.join(dir_path,
    ...
    ComNeRFTest_config_file = os.path.join(id_dir, 'body_TorsoNeRFTest_config.txt')
    with open(ComNeRFTest_config_file, 'w') as file:
        file.write('expname = ' + id + '_com\n')
        file.write('datadir = ' + os.path.join(dir_path, 'dataset', id) + '\n')
        file.write('basedir = ' + os.path.join(dir_path,
        ...
        file.write('far = ' + str(mean_z+0.4) + '\n')
        file.write('with_test = ' + str(1) + '\n')
        file.write('test_pose_file = body_transforms_val.json' + '\n')
